...
function updateProfile($user_id, $avatar, $birth_date, $country){
  global $conn;
  $message = array();
  $query = "UPDATE user
    SET user.avatar = ?, user.birth_date =? , user.country = ?
    WHERE user.id = ?";
  $statement = $conn->prepare($query);
  $statement->bind_param("sssi", $avatar, $birth_date, $country ,$user_id);
  if ($statement->execute()) {
